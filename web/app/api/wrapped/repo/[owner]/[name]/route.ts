...
import { recordAndOdds } from "@/lib/personaStats";
...
export async function GET(
  ...
  try {
    const stats = await getWrappedRepoStats(owner, name, opts);
    if (stats.persona) {
      const odds = await recordAndOdds(stats.persona.id);
    ...
    return new NextResponse(
      `<!doctype html><meta charset=utf-8><body style="font-family:system-ui;background:#0b0b12;color:#f5f5fa;display:grid;place-items:center;height:100vh;margin:0"><div style="text-align:center"><h1>${
        code === 404 ? "No such repo" : "Couldn't load that wrapped"
