#!/usr/bin/env node
// compute-persona.mjs — derive headline metrics + a persona archetype (with rarity)
// from a stats.json produced by extract-git-stats.mjs / extract-github-stats.mjs.
//
// Usage: node compute-persona.mjs [statsFile] [--out FILE]
// Defaults: reads ./dev-wrapped-stats.json, writes the enriched object back in place.
//
// KEEP THE PERSONAS TABLE IN SYNC with web/lib/persona.ts.
...
// Higher score wins. rare/legendary use larger multipliers so they win WHEN met.
const PERSONAS = [
  // legendary
  {
...
function main() {
  ...
  console.log(`${winner.emoji}  ${winner.title} [${winner.rarity}] — ${winner.evidence(signals)}`);
}
